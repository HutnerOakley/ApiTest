#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Time    : 2024-02-22 14:43
# @Name    : test_machinetab_query.py
# @Description   : 机型tab查询接口
# @Author  : Hunter
...
machinetab_yaml_path = os.path.join(os.path.split(os.path.dirname(os.path.dirname(__file__)))[0], 'test_data',
                                    'TabManagement', 'machinetab_query.yaml')
...
class TestMachinetab:

    @allure.story("机型tab查询接口")
    @pytest.mark.parametrize('case_info', eval(case_data), ids=[i['api_name'] for i in re_data])
    def test_machinetab_query(self, case_info):
